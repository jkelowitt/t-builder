...
for titr in titrations:
    titr.write_titration_data(
        title=f"test_data/{titr.aname}_{titr.tname}_titration_data".replace(" ", "_").lower(), file_headers=True
    )
    titr.write_alpha_data(
        title=f"test_data/{titr.aname}_{titr.tname}_alpha_data".replace(" ", "_").lower(), file_headers=True
    )
    titr.write_analysis_data(
        title=f"test_data/{titr.aname}_{titr.tname}_analysis_data".replace(" ", "_").lower(), file_headers=True
    )
